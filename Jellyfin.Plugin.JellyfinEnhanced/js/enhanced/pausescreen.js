...
(function (JE) {
  ...
  JE.initializePauseScreen = function () {
    function log(type, msg, errorDetails = null) {
      if (type === "debug" && !JellyfinPauseScreen.debug) return;
      const prefix = "🪼 Jellyfin Enhanced (PauseScreen): ";
      if (errorDetails) {
    ...
    if (!JE.currentSettings.pauseScreenEnabled) {
      log("info", "Custom Pause Screen is disabled.");
      return;
    }
    ...
    class JellyfinPauseScreen {
      ...
      createOverlay() {
        ...
        const tryResume = (event) => {
          if (
            ...
            event.target === this.overlayContent
          ) {
        ...
        this.overlay.addEventListener("touchstart", tryResume, {
          passive: true,
        });
            ...
            if (this.overlay.getAttribute("aria-hidden") === "false") {
              if (e.code === "Space" || e.code === "Enter") {
                ...
                if (this.currentVideo && this.currentVideo.paused) {
                  this.currentVideo
                    .play()
                    .catch((err) => log("warn", "Play() blocked:", err));
                }
              }
              // Keep Tab inside
              if (e.code === "Tab") {
      ...
      async fetchItemInfo(itemId) {
        ...
        try {
          ...
          if (!record) {
            const itemResp = await this.fetchWithRetry(
              ApiClient.getUrl(`/Items/${itemId}`),
              {
                headers: {
      ...
      async displayItemInfo(item, domain, itemId) {
        ...
        this.overlayDetails.innerHTML = [
          year && `<span>${year}</span>`,
          rating &&
            `<span class="mediaInfoOfficialRating" rating="${rating}">${rating}</span>`,
        ...
        this.overlayPlot.textContent =
          item.Overview || JE.t("pausescreen_no_description");
        ...
        if (backdropURL)
          this.overlayBackdrop.style.backgroundImage = `url("${backdropURL}")`;
      ...
      updateProgressStatic() {
        ...
        const cur = Number.isFinite(this.currentVideo.currentTime)
          ? this.currentVideo.currentTime
          : 0;
        const dur = Number.isFinite(this.currentVideo.duration)
      ...
      setProgress(current, duration) {
        ...
        const percentageEl = this.progressMeta.querySelector(
          ".progress-percentage",
        );
        ...
        if (percentageEl) {
          percentageEl.textContent = JE.t("pausescreen_watched_percent", {
        ...
        if (endsAtEl) {
          if (duration > 0 && current < duration) {
            ...
            endsAtEl.textContent = JE.t("pausescreen_ends_at", {
      ...
      getLogoUrls(item, domain, itemId) {
        ...
        if (item.ImageTags?.Logo) {
          urls.push(
            `${domain}/Items/${itemId}/Images/Logo?tag=${item.ImageTags.Logo}`,
          );
        }
        if (item.ParentId)
      ...
      getBackdropUrls(item, domain, itemId) {
        const urls = [`${domain}/Items/${itemId}/Images/Backdrop`];
        if (item.ParentId)
          urls.push(`${domain}/Items/${item.ParentId}/Images/Backdrop`);
        if (item.SeriesId)
          urls.push(`${domain}/Items/${item.SeriesId}/Images/Backdrop`);
        return urls;
      }
    ...
    log("info", "Custom Pause Screen initialized.");
  };
